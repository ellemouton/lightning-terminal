...
func (s *SQLStore) CreateSession(ctx context.Context, sess *Session) error {
	...
	err := s.db.ExecTx(ctx, &writeTxOpts, func(db SQLQueries) error {
		id, err := db.InsertSession(ctx, sqlc.InsertSessionParams{
			LegacyID:        sess.ID[:],
			Label:           sess.Label,
			SessionState:    int16(sess.State),
			SessionType:     int16(sess.Type),
			Expiry:          sess.Expiry,
		...
		if errors.Is(err, sql.ErrNoRows) {
			return ErrUnknownLinkedSession
		} else if err != nil {
			return err
		}
		...
		if err != nil {
			return fmt.Errorf("unable to get linked sessions: %w",
				err)
		}
		...
		for _, linkedSession := range linkedSessions {
			...
			if State(linkedSession.SessionState) == StateCreated ||
				State(linkedSession.SessionState) == StateInUse {

				return ErrSessionsInGroupStillActive
			}
		}
		...
		if err != nil {
			return fmt.Errorf("unable to set group ID: %v", err)
		}
...
func (s *SQLStore) GetSession(ctx context.Context, key *btcec.PublicKey) (
	*Session, error) {
	...
	err := s.db.ExecTx(ctx, &readTxOpts, func(db SQLQueries) error {
		...
		if err != nil {
			return fmt.Errorf("unable to unmarshal session: %v",
				err)
		}
	...
	return s.db.ExecTx(ctx, &writeTxOpts, func(db SQLQueries) error {
		...
		return s.db.UpdateSessionState(
			ctx, sqlc.UpdateSessionStateParams{
				ID:           dbSess.ID,
				SessionState: int16(StateRevoked),
			},
		)
	...
	return s.db.ExecTx(ctx, &writeTxOpts, func(db SQLQueries) error {
		return db.SetRemotePublicKeyByLocalPublicKey(
			ctx, sqlc.SetRemotePublicKeyByLocalPublicKeyParams{
				RemotePublicKey: remotePubKey.SerializeCompressed(),
				LocalPublicKey:  localPubKey.SerializeCompressed(),
			},
		)
...
func unmarshalSession(ctx context.Context, db SQLQueries,
	...
	return &Session{
		ID:                legacyID,
		Label:             dbSess.Label,
		State:             State(dbSess.SessionState),
		Type:              Type(dbSess.SessionType),
