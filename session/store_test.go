...
// TestLinkedSessions tests that the session-ID-to-group-ID and
...
func TestLinkedSessions(t *testing.T) {
	...
	s1 := newSession(t, "session 1", nil)
	s2 := newSession(t, "session 2", &s1.GroupID)
	s3 := newSession(t, "session 3", &s2.GroupID)
	...
	s4 := newSession(t, "session 4", nil)
	s5 := newSession(t, "session 5", &s4.GroupID)
