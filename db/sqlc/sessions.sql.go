...
const setSessionRemotePublicKeyByLocalPublicKey = `-- name: SetSessionRemotePublicKeyByLocalPublicKey :exec
UPDATE sessions
SET remote_public_key = $1
WHERE local_public_key = $2
`

type SetSessionRemotePublicKeyByLocalPublicKeyParams struct {
	RemotePublicKey []byte
	LocalPublicKey  []byte
}

func (q *Queries) SetSessionRemotePublicKeyByLocalPublicKey(ctx context.Context, arg SetSessionRemotePublicKeyByLocalPublicKeyParams) error {
	_, err := q.db.ExecContext(ctx, setSessionRemotePublicKeyByLocalPublicKey, arg.RemotePublicKey, arg.LocalPublicKey)
	return err
}
