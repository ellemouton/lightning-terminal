...
-- name: ListActionsByGroupIDAndState :many
SELECT a.*
FROM actions a
...
-- name: ListActionsByGroupIDStateAndFeatureName :many
SELECT a.*
FROM actions a
 JOIN sessions s ON a.session_id = s.id
...
WHERE (a.session_id = sqlc.narg('session_id') OR sqlc.narg('session_id') IS NULL)
  ...
  AND (a.action_state = sqlc.narg('action_state') OR sqlc.narg('action_state') IS NULL)
