...
func (s *sessionRpcServer) ListActions(ctx context.Context,
	...
	var (
		listOptions = []firewalldb.ListActionOption{
			firewalldb.WithFeatureName(req.FeatureName),
			firewalldb.WithActorName(req.ActorName),
			firewalldb.WithMethodName(req.MethodName),
			firewalldb.WithState(state),
		}
	...
	if req.SessionId != nil {
		...
		addOption(firewalldb.WithSessionID(sessionID))
		...
		addOption(firewalldb.WithGroupID(groupID))
	}

	if req.EndTimestamp != 0 {
		addOption(firewalldb.WithEndTime(
			time.Unix(int64(req.EndTimestamp), 0)),
		)
	}

	if req.StartTimestamp != 0 {
		addOption(firewalldb.WithStartTime(
