...
// testDisablingSubServers will restart LiT with the loop, pool & faraday
// sub-servers disabled and will then run the test suite against it.
func testDisablingSubServers(ctx context.Context, net *NetworkHarness,
	t *testing.T, test testSuite, node *HarnessNode) {

	// Restart the Lit node with all the sub-servers disabled.
	err := net.RestartNode(
...
func runGRPCAuthTest(t *testing.T, hostPort, tlsCertPath, macPath string,
	...
	require.Error(t, err)
	if disabled {
		require.Contains(t, err.Error(), disabledErr)
	} else {
	...
	if disabled {
		...
		require.Contains(t, err.Error(), "cannot get macaroon: root "+
			"key with")
	}
...
func runUIPasswordCheck(t *testing.T, hostPort, tlsCertPath, uiPassword string,
	...
	err3 := strings.Contains(errStr, disabledErr)
	require.True(t, err1 || err2 || err3, "wrong UI password and dummy mac")
...
func runLNCAuthTest(t *testing.T, rawLNCConn grpc.ClientConnInterface,
	...
	if !callAllowed {
		...
		if disabled {
			require.Contains(t, err.Error(), "unknown gRPC web "+
				"request")
		} else {
	...
	if disabled {
		require.Error(t, err)
		require.Contains(t, err.Error(), "unknown gRPC web request")
